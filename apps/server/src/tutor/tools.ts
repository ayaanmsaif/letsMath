...
const DESCRIPTIONS: Record<BoardOpName, string> = {
  ...
  draw_diagram:
    "Draw a figure or a graph from scratch: a triangle for the question, a labelled right angle, a construction, or y = sin x on axes. For a figure, give named points (A, B, C) in maths coordinates and say what to draw between them. For a graph, give the axes range you want and the functions to plot, written plainly as sin(x) or x^2-3 — the board samples the real curve, so never list the points of a curve yourself. Either way the board works out every line, arc, tick and label, so your numbers only need to be mathematically right, not positioned on screen. It goes in clear space near what you name in near.",
